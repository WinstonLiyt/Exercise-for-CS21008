...
# from rnn import word_embedding, RNN_model
...
def run_training():
    ...
    for epoch in range(30):
        ...
        for batch in range(n_chunk):
            ...
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm(rnn_model.parameters(), 1)  # 解决梯度爆炸
            optimizer.step()

            if batch % 20 == 0:
                today = datetime.date.today()
                file_name = "poem_generator_rnn_" + str(today)+'ts'
                torch.save(rnn_model.state_dict(), file_name)
...
def gen_poem(begin_word):
    ...
    today = datetime.date.today()
    file_name = "poem_generator_rnn_" + str(today) +'ts'
    rnn_model.load_state_dict(torch.load(file_name))
