...
def weights_init(m):
    ...
    if classname.find("Linear") != -1:
        ...
        w_bound = np.sqrt(6.0 / (fan_in + fan_out))  # He初始化计算权重
...
class RNN_model(nn.Module):
    ...
    def forward(self, sentence, is_test=False):
        ...
        # ???
        h0 = torch.zeros(2, batch_input.size(0), self.lstm_dim).to(device)
        c0 = torch.zeros(2, batch_input.size(0), self.lstm_dim).to(device)
